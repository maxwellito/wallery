...
function WalleryBuilder (settings) {
	...
	if (settings['unit'] === undefined		||
		...
		throw "WalleryBuilder: settings aren't defined";
	
	// Check unit
	if (settings['unit'] <= 0)
		throw "WalleryBuilder: unit is not valid";
	
	// Check the map size
	if (settings['unit'] > settings['mapWidth'] || settings['unit'] > settings['mapHeight'])
		throw "WalleryBuilder: unit is bigger than the map";
 ...
 */
WalleryBuilder.prototype.rendering = function (tplFunction) {
